...
public class NSG_DataContentsTests extends DataContentsTests {
    ...
    private double checkFeatureBounds( String _tableName, String _boundsColumn )
        ...
        try (final Statement statement = this.databaseConnection.createStatement();
            ...
            Assert.assertTrue( resultSet.next(), "Invalid no result from table: <gpkg_geometry_columns>" );
        ...
        try (final Statement statement = this.databaseConnection.createStatement();
                                final ResultSet resultSet = statement.executeQuery( queryStr )) {
            Assert.assertTrue( resultSet.next(), "Invalid no result from " + _func1 + "(" + _colName + ") in table:  "
                                                 + _tableName );
