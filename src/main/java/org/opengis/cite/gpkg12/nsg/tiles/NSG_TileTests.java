...
public class NSG_TileTests extends TileTests {
    ...
    public void NSG_DataValidity()
        ...
        try (final Statement statement = this.databaseConnection.createStatement();
            ...
            while ( resultSet.next() ) {
                ...
                tileQuery = "SELECT zoom_level, tile_width, tile_height, pixel_x_size, pixel_y_size FROM gpkg_tile_matrix WHERE table_name=\'"
                            + _tabNam + "\' ORDER BY zoom_level;";
                    ...
                    while ( tileResultSet.next() ) {
                        ...
                        Assert.assertTrue( ( ( _zoom >= _firstZoom ) && ( _zoom <= _lastZoom ) ),
                                           MessageFormat.format( "The gpkg_tile_matrix contains an invalid zoom_level: {0} for {1}, should be between {2} and {3}",
                                                                 Integer.toString( _zoom ), _tabNam,
                                                                 Integer.toString( _firstZoom ),
                        ...
                        Assert.assertTrue( ( _tileWidth == 256 ),
                                           MessageFormat.format( "The gpkg_tile_matrix contains an invalid tile_width: {0} for {1}, should be 256",
                                                                 Integer.toString( _tileWidth ), _tabNam ) );
                        ...
                        Assert.assertTrue( ( _tileHeight == 256 ),
                                           MessageFormat.format( "The gpkg_tile_matrix contains an invalid tile_height: {0} for {1}, should be 256",
                                                                 Integer.toString( _tileHeight ), _tabNam ) );
                        ...
                        Assert.assertTrue( ( ( _zoom == _firstZoom ) || ( _delta < this.tolerance ) ),
                                           MessageFormat.format( "The gpkg_tile_matrix contains an invalid pixel_x_size: {0} for {1}",
                                                                 String.format( "%.10f", _pixelSzX ), _tabNam ) );
                        ...
                        Assert.assertTrue( ( ( _zoom == _firstZoom ) || ( _delta < this.tolerance ) ),
                                           MessageFormat.format( "The gpkg_tile_matrix contains an invalid pixel_y_size: {0} for {1}",
                                                                 String.format( "%.10f", _pixelSzY ), _tabNam ) );
                    }
                    tileResultSet.close();
                    tileStatement.close();
     ...
     * 
     * --- NSG Req 21: Every tile_data tile in every table_name tile pyramid data table shall have a width and height of
     * 256 pixels.
     */

    @Test(groups = { "NSG" }, description = "NSG Req 20 & 21 (Tile widths and heights)")
    public void NSG_TileSizeTests()
        ...
        try (final Statement statement = this.databaseConnection.createStatement();
            ...
            while ( resultSet.next() ) {
                ...
                tableNames.add( _tabNam + ": (tile_width)" + resultSet.getInt( "tile_width" ) + ", (tile_height)"
                                + resultSet.getInt( "tile_height" ) );
            }
            resultSet.close();
            statement.close();
     ...
     * 2.2.3. It SHALL NOT contain pixel sizes that vary by irregular intervals or by regular intervals other than a
    ...
    public void NSG_PixelsSizeTests()
                            throws SQLException {
        String queryStr = "SELECT table_name FROM gpkg_contents WHERE data_type=\'tiles\';";
     ...
     * position of each tile in the tile pyramid data table.
     */

    @Test(groups = { "NSG" }, description = "NSG Req 23 (bounding box in gpkg_tile_matrix_set)")
    public void NSG_BoundingBoxTests()
                            throws SQLException {
        ...
        try (final Statement statement = this.databaseConnection.createStatement();
            ...
            while ( resultSet.next() ) {
                ...
                if ( srsID.equals( "3395" ) ) {
                    mbr = new double[] { -20037508.342789244D, -20037508.342789244D, 20037508.342789244D,
                                        20037508.342789244D };
                ...
                Assert.assertTrue( ( _val == mbr[0] ),
                                   MessageFormat.format( "The gpkg_tile_matrix_set contains an invalid min_x value: {0} for table {1} (should be {2})",
                                                         Double.valueOf( _val ), _tabNam, Double.valueOf( mbr[0] ) ) );
                ...
                Assert.assertTrue( ( _val == mbr[1] ),
                                   MessageFormat.format( "The gpkg_tile_matrix_set contains an invalid min_y value: {0} for table {1} (should be {2})",
                                                         Double.valueOf( _val ), _tabNam, Double.valueOf( mbr[1] ) ) );
                ...
                Assert.assertTrue( ( _val == mbr[2] ),
                                   MessageFormat.format( "The gpkg_tile_matrix_set contains an invalid max_x value: {0} for table {1} (should be {2})",
                                                         Double.valueOf( _val ), _tabNam, Double.valueOf( mbr[2] ) ) );
                ...
                Assert.assertTrue( ( _val == mbr[3] ),
                                   MessageFormat.format( "The gpkg_tile_matrix_set contains an invalid max_y value: {0} for table {1} (should be {2})",
                                                         Double.valueOf( _val ), _tabNam, Double.valueOf( mbr[3] ) ) );
            }
            resultSet.close();
            statement.close();
        }
